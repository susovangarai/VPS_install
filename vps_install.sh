...
cd /root/go/src/github.com/tillson/git-hound/
go build
cd .. 
mv git-hound/ /root/Tools/hacks/anti-burl/
cd /root/Tools/hacks/anti-burl
go build
rm readme.md
alias antiburl=/root/Tools/hacks/anti-burl/anti-burl
cd /root/Tools/hacks/qsreplace
go build
cd ..
cd /root/Tools/kxss
go build
cd /root/Tools/kxss
go build
sh root/Tools/Brutx/install.sh
sh /root/Tools/Osmedeus/install.sh

echo 'source /root/Tools/gf/gf-completion.bash' >> ~/.bashrc;
echo "
...
alias qsreplace=/root/Tools/hacks/qsreplace
" >> /root/.bashrc;
echo "
export GOROOT=/usr/local/go
export GOPATH=$HOME/go
